 ...
 *   variants  [{ name, note, code? }] only where behaviour differs
...
WHERE u.status = 'active';`,
    ...
    variants: [
      { name: 'SQL Server', note: 'Also supports alias = expression, e.g. SELECT contact = u.email.' },
      { name: 'Oracle', note: 'AS is not allowed for table aliases: write FROM users u.' },
    ],
...
ORDER BY placed_at DESC, id DESC;`,
    ...
    variants: [
      { name: 'SQL Server', note: 'No NULLS FIRST/LAST: emulate with ORDER BY CASE WHEN col IS NULL THEN 1 ELSE 0 END, col.' },
    ],
...
LIMIT 20 OFFSET 40;`,
    ...
    variants: [
      { name: 'PostgreSQL / MySQL / SQLite', note: 'LIMIT n OFFSET m.' },
      { name: 'SQL Server', note: 'SELECT TOP (20) ..., or OFFSET ... ROWS FETCH NEXT ... ROWS ONLY, which requires ORDER BY.' },
      { name: 'Oracle', note: '12c and later support FETCH FIRST n ROWS ONLY; older versions wrap the query and filter on ROWNUM.' },
    ],
...
FROM orders;`,
    ...
    variants: [
      { name: 'MySQL', note: 'IF(cond, a, b) is a shorter two-branch form.' },
      { name: 'PostgreSQL', note: 'FILTER (WHERE ...) reads better than CASE inside an aggregate.' },
    ],
...
FROM ...;`,
    ...
    variants: [
      { name: 'MySQL', note: 'IFNULL(a, b) is the two-argument version.' },
      { name: 'SQL Server', note: 'ISNULL(a, b) is two-argument and takes the type of its first argument; COALESCE is the portable choice.' },
      { name: 'Oracle', note: 'NVL(a, b) and NVL2(a, b, c).' },
    ],
...
FROM orders;`,
    ...
    variants: [
      { name: 'SQL Server', note: 'CONVERT(type, expr, style) adds date-format styles that CAST lacks.' },
      { name: 'Oracle', note: 'TO_NUMBER, TO_DATE and TO_CHAR are the everyday converters.' },
    ],
...
WHERE email LIKE 'sourabh%';`,
    ...
    variants: [
      { name: 'PostgreSQL', note: 'LIKE is case-sensitive, ILIKE is not. pg_trgm plus a GIN index makes contains-patterns indexable.' },
      { name: 'MySQL', note: 'LIKE follows the column collation, so it is usually case-insensitive already.' },
      { name: 'Oracle / SQL Server', note: 'Compare UPPER(col) to an uppercase pattern, or use a case-insensitive collation.' },
    ],
...
WHERE country IS NULL;`,
    ...
    variants: [
      { name: 'PostgreSQL', note: 'a IS NOT DISTINCT FROM b treats NULL = NULL as true.' },
      { name: 'MySQL', note: 'The <=> operator is NULL-safe equality.' },
      { name: 'Oracle', note: 'An empty string is stored as NULL, so comparing a column to an empty string never matches.' },
    ],
...
WHERE email !~ '^[^@[:space:]]+@[^@[:space:]]+\\.[a-z]{2,}$';`,
    ...
    variants: [
      { name: 'PostgreSQL', note: 'The operators are ~ (match), ~* (case-insensitive) and !~ (no match).' },
      { name: 'SQL Server', note: 'No native regex in older versions: use LIKE or a CLR function.' },
    ],
...
CROSS JOIN LATERAL (
  ...
  LIMIT 3
    ...
    variants: [
      { name: 'PostgreSQL / Oracle', note: 'LATERAL (Oracle also has CROSS APPLY and OUTER APPLY).' },
      { name: 'SQL Server', note: 'CROSS APPLY and OUTER APPLY.' },
      { name: 'MySQL', note: 'LATERAL is supported from 8.0.14.' },
    ],
...
ORDER BY users DESC;`,
    ...
    variants: [
      { name: 'MySQL', note: 'With ONLY_FULL_GROUP_BY disabled it will return an arbitrary value for unaggregated columns. Leave the mode on.' },
    ],
...
ORDER BY 1;`,
    ...
    variants: [
      { name: 'MySQL', note: 'COUNT(DISTINCT a, b) counts distinct combinations, and skips rows where any part is NULL.' },
    ],
...
GROUP BY user_id;`,
    ...
    variants: [
      { name: 'PostgreSQL / SQLite', note: 'FILTER (WHERE ...) is the readable standard form.' },
      { name: 'MySQL / SQL Server / Oracle', note: 'No FILTER clause. Use the CASE form.' },
    ],
...
WHERE r.position <= 10;`,
    ...
    variants: [
      { name: 'PostgreSQL', note: 'WITH name AS MATERIALIZED (...) pins the result so it is computed once.' },
      { name: 'MySQL', note: 'Supported from 8.0. Earlier versions need derived tables.' },
    ],
...
SELECT * FROM reports ORDER BY depth, name;`,
    ...
    variants: [
      { name: 'Oracle', note: 'Also offers CONNECT BY PRIOR, an older and more compact hierarchy syntax.' },
    ],
...
SELECT email FROM users;`,
    ...
    variants: [
      { name: 'Oracle', note: 'EXCEPT is spelled MINUS.' },
    ],
...
RETURNING id;`,
    ...
    variants: [
      { name: 'PostgreSQL / SQLite / MariaDB', note: 'RETURNING gives back generated ids in the same statement.' },
      { name: 'SQL Server', note: 'OUTPUT INSERTED.id serves the same purpose.' },
      { name: 'MySQL', note: 'LAST_INSERT_ID() returns the first id of the batch.' },
    ],
...
WHERE status = 'active'
  AND id NOT IN (SELECT user_id FROM orders WHERE placed_at > CURRENT_DATE - INTERVAL '2 years');`,
    ...
    variants: [
      { name: 'PostgreSQL / SQLite', note: 'RETURNING * shows exactly which rows changed.' },
      { name: 'MySQL', note: 'The safe-updates setting refuses an UPDATE without a key in WHERE. Leave it on for interactive sessions.' },
    ],
...
DO UPDATE SET hits = daily_counters.hits + 1;`,
    ...
    variants: [
      { name: 'SQL Server / Oracle', note: 'Use MERGE. On SQL Server, take a HOLDLOCK on the target or concurrent MERGEs can still deadlock or duplicate.' },
    ],
...
COMMIT;`,
    ...
    variants: [
      { name: 'PostgreSQL / MySQL 8 / Oracle', note: 'FOR UPDATE SKIP LOCKED is supported.' },
      { name: 'SQL Server', note: 'Use WITH (UPDLOCK, READPAST, ROWLOCK) for the same effect.' },
    ],
  ...
  placed_at  TIMESTAMPTZ NOT NULL DEFAULT NOW()
    ...
    variants: [
      { name: 'PostgreSQL', note: 'GENERATED ALWAYS AS IDENTITY is the modern alternative to SERIAL.' },
      { name: 'MySQL', note: 'BIGINT AUTO_INCREMENT, and use utf8mb4 for real Unicode.' },
      { name: 'SQL Server', note: 'BIGINT IDENTITY(1,1), DATETIME2 rather than DATETIME.' },
    ],
...
-- ON DELETE CASCADE | SET NULL | RESTRICT | NO ACTION`,
  ...
  ON DELETE RESTRICT;`,
    ...
    variants: [
      { name: 'MySQL', note: 'InnoDB creates an index on the FK column automatically; MyISAM ignores foreign keys entirely.' },
      { name: 'PostgreSQL', note: 'No automatic index. Add it yourself.' },
    ],
...
GROUP BY u.email;`,
    ...
    variants: [
      { name: 'PostgreSQL', note: 'EXPLAIN (ANALYZE, BUFFERS, FORMAT TEXT). Paste the output into explain.dalibo.com to read it visually.' },
      { name: 'MySQL', note: 'EXPLAIN FORMAT=JSON gives cost details; EXPLAIN ANALYZE runs it.' },
      { name: 'SQL Server', note: 'SET STATISTICS IO, TIME ON, plus the actual execution plan.' },
    ],
...
FROM users;`,
    ...
    variants: [
      { name: 'PostgreSQL', note: 'SPLIT_PART(s, delim, n) and STRING_TO_ARRAY are the splitting tools.' },
      { name: 'MySQL', note: 'SUBSTRING_INDEX(s, delim, n); no || operator by default.' },
      { name: 'SQL Server', note: 'Use + to concatenate, or CONCAT; STRING_SPLIT for splitting.' },
    ],
...
CREATE INDEX idx_events_payload ON events USING GIN (payload jsonb_path_ops);`,
    ...
    variants: [
      { name: 'MySQL', note: 'Index a JSON path by adding a generated column and indexing that.' },
    ],
